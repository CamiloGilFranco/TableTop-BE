...
import { 
 ...
 export const getAllRestaurantsController = async (
  ...
  try {
    ...
    res.status(200).json({message: 'Restaurants retrieved successfully', data: restaurants});
  } catch (error: any) {
 ...
 export const getAllRestaurantByIdController = async (
  ...
  try {
    ...
    res.status(200).json({ message: 'Restaurant with that ID found!', data: restaurant });
  } catch (error: any) {
    res.status(500).json({message: error.message}); 
  }
 }
